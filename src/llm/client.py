...
from prompts import get_summary_prompt, get_memory_update_prompt, get_complete_memory_json_prompt
...
class LLMClient:
    ...
    def _build_chat_messages(
        ...
        messages.append({
            "role": "system",
            "content": get_cacheable_system_prompt(recent_summaries)
        })
        ...
        messages.append({
            "role": "system",
            "content": f"**Current Time**: {get_current_time_str()}"
        })
        ...
        if current_context:
            ...
            emphasized_message = {
                ...
                "content": f"⚡ [Current Request]\n{current_user_message['content']}\n\nPlease respond to the above request based on all context provided."
            }
    ...
    def generate_memory_update(
        ...
        prompt = get_memory_update_prompt(conv_text, first_timestamp, last_timestamp)
            ...
            if "no running event loop" in str(e).lower():
                ...
                async def get_update() -> str:
                    ...
                    response = await self._tool_client.chat.completions.create(
                        model=self.tool_model,
                        messages=[
                            {"role": "system", "content": "You generate JSON patches for memory updates."},
                            {"role": "user", "content": prompt}
                        ]
                    )
    ...
    def generate_complete_memory_json(
        ...
        prompt = get_complete_memory_json_prompt(conv_text, current_seele_json, error_message, first_timestamp, last_timestamp)
            ...
            if "no running event loop" in str(e).lower():
                ...
                async def get_complete_json() -> str:
                    ...
                    response = await self._tool_client.chat.completions.create(
                        model=self.tool_model,
                        messages=[
                            {"role": "system", "content": "You generate complete seele.json objects for memory updates."},
                            {"role": "user", "content": prompt}
                        ]
                    )
