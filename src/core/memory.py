...
class MemoryManager:
    ...
    def _restore_context_from_session(self, session_id: int) -> None:
        ...
        if existing_summaries:
            ...
            for summary_row in reversed(recent_summaries):
                self.context_window.add_summary(
                    summary=summary_row["summary"], summary_id=summary_row["summary_id"]
                )
            logger.info(
                f"Restored {len(recent_summaries)} existing summaries to context window"
            )

        # Step 2: Get all unsummarized conversations and load them into context window
        ...
        # Load ALL unsummarized messages to ensure correct counting for summarization trigger
    ...
    def new_session(self) -> int:
        ...
        if old_session:
            ...
            if remaining_messages:
                ...
                first_timestamp = int(
                    get_current_timestamp() - len(remaining_messages) * 60
                )
    ...
    async def new_session_async(self) -> int:
        ...
        if old_session:
            ...
            if remaining_messages:
                ...
                await self._update_long_term_memory_async(
                    summary_id, remaining_messages
                )
    ...
    def process_user_input(
        ...
        formatted_conversations = self.retriever.format_conversations_for_prompt(
            conversations
        )
    ...
    async def process_user_input_async(
        ...
        # Try to find user_input_embedding and last_bot_embedding from context window
        messages = self.context_window.get_messages()
        last_bot_embedding = None

        # Look for last assistant message with embedding
        for msg in reversed(messages):
            if msg.role == "assistant" and msg.embedding:
                last_bot_embedding = msg.embedding
                break

        # If user_input_embedding not provided, try to find it in the last message
    ...
    def add_user_message(
        ...
        self.context_window.add_message(role="user", text=text, embedding=embedding)
    ...
    def _check_and_create_summary(
        ...
        # Get real timestamps from database for the messages being summarized
        session_id = self.get_current_session_id()
        conversations = self.db.get_conversations_by_session(session_id)
        message_count = len(messages_to_summarize)

        if conversations and len(conversations) >= message_count:
            # Use real timestamps from the first N conversations (oldest ones being summarized)
            first_timestamp = conversations[0]["timestamp"]
            last_timestamp = conversations[message_count - 1]["timestamp"]
        else:
            # Fallback if no matching records found
            first_timestamp = get_current_timestamp()
            last_timestamp = get_current_timestamp()
            logger.warning(
                "Could not get real timestamps from database, using current time"
            )
    ...
    async def _update_long_term_memory_async(
        ...
        try:
            ...
            success = await self.update_long_term_memory_async(
