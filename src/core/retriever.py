...
class MemoryRetriever:
    ...
    def retrieve_related_memories(
        ...
        Args:
            ...
            exclude_summary_ids: Optional list of summary_ids to exclude (e.g., recent summaries already in context)
        """
        ...
        if last_bot_message:
            bot_embedding = self.embedding_client.get_embedding(last_bot_message)
        ...
        for r in query_results:
            ...
            summaries.append(RetrievedSummary(
        ...
        if summaries:
            for summary in summaries:
                summary_embedding = self.embedding_client.get_embedding(summary.summary)
                conv_results = self.db.search_conversations(summary_embedding, limit=Config.RECALL_CONV_PER_SUMMARY)
                
                for r in conv_results:
                    conversation_id, timestamp, role, text, distance = r
                    conversations_result.append(RetrievedConversation(
        ...
        if self.reranker_client.is_enabled() and summaries:
            summary_docs = [
                {"text": s.summary, "summary_id": s.summary_id, 
                 "first_timestamp": s.first_timestamp, "last_timestamp": s.last_timestamp}
                for s in summaries
            ]
            reranked_summaries = self.reranker_client.rerank(
                query=query,
                documents=summary_docs,
                top_n=Config.RERANK_TOP_SUMMARIES
            )
            summaries_result = [
                RetrievedSummary(
        ...
        if self.reranker_client.is_enabled() and conversations_result:
            conv_docs = [
                {"text": c.text, "conversation_id": c.conversation_id, 
                 "timestamp": c.timestamp, "role": c.role}
                for c in conversations_result
            ]
            reranked_convs = self.reranker_client.rerank(
                query=query,
                documents=conv_docs,
                top_n=Config.RERANK_TOP_CONVS
            )
            conversations_result = [
    ...
    async def retrieve_related_memories_async(
        ...
        Args:
            ...
            last_bot_message: Optional last bot message for dual-query
            query_embedding: Optional pre-computed embedding to avoid re-vectorization
            exclude_summary_ids: Optional list of summary_ids to exclude (e.g., recent summaries already in context)
        """
        from config import Config
        ...
        if query_embedding is None:
            ...
            logger.debug("Reusing provided query_embedding (avoiding re-vectorization)")
        ...
        if last_bot_message:
            bot_embedding = await self.embedding_client.get_embedding_async(last_bot_message)
            bot_results = self.db.search_summaries(
                bot_embedding, 
                limit=Config.RECALL_SUMMARY_PER_QUERY,
                exclude_ids=exclude_summary_ids
        ...
        if summaries:
            for summary in summaries:
                summary_embedding = await self.embedding_client.get_embedding_async(summary.summary)
                conv_results = self.db.search_conversations(summary_embedding, limit=Config.RECALL_CONV_PER_SUMMARY)
    ...
    def format_summaries_for_prompt(self, summaries: List[RetrievedSummary]) -> List[str]:
        formatted = []
    ...
    def format_conversations_for_prompt(self, conversations: List[RetrievedConversation]) -> List[str]:
        formatted = []
