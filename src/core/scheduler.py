...
class TaskScheduler:
    ...
    async def _execute_task(self, task_data: Dict[str, Any]) -> None:
        ...
        if trigger_type == "once":
            self.db.update_task_next_run(task_id, next_run_at=0, last_run_at=last_run_at)
            ...
            self.db.update_task_next_run(task_id, next_run_at=next_run_at, last_run_at=last_run_at)
