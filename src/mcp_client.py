...
class MCPClient:
    ...
    async def __aenter__(self):
        ...
        # 使用 fastmcp.Client 连接到所有配置的服务器，直接传入 roots
...
def create_mcp_client(
    config_path: str = "mcp_servers.json", roots: Optional[List[str]] = None
) -> MCPClient:
    """创建 MCP 客户端实例的工厂函数

    Args:
        config_path: MCP服务器配置文件路径
        roots: 可选的 roots 路径列表，如果不提供则使用数据目录
